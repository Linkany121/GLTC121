 ...
 * -----------------------------------------------------------------------------
 *
 *  站立右键     → 施术（选术环开着且对准槽 → 选槽；未对准 → 关环并施术）
 *  站立左键     → 选术环开着选槽；否则 → 术式左键钩子（handleSpellLeftClick）
 *  蹲下右键     → 开关选术环；成功开环时可触发道具 onSneakUse；绝不施术
 *  蹲下左键     → 不触发术式逻辑
 *
 *  切选中术式 / 开选术环 / 换手持 / 退服 → 清术式会话（实体+任务）
 ...
 *  [ ] cast 返回 true/false（false 会退还粒子）
 *
 *  瞬时术式（右键一次完事）：
 ...
 *  粒子：cast 前由核心扣除；成功播报「消耗 x 粒子 使用 x环术式 xxx」
 *
 *  等级 vs 环数（核心 resolveCastCost）：
 *    等级 > 环数 → 消耗 ×0.5（最低 1）
 *    等级 < 环数 → 侵蚀 = 环数-等级；消耗 ×侵蚀；自伤 = 侵蚀×20% 最大生命
 *
 *  伤害 API（UTIL / _工具.js）：
 ...
 *    onAfterCast(player)     施术成功后（粒子已扣）
 *    onSneakUse(player)      蹲下右键成功开选术环时（读 metadata token 防双触发）
 *
 * -----------------------------------------------------------------------------
 ...
 *  ❌ cast 不 return true 导致误报「施术失败」并退粒子
 *  ❌ 重复扣粒子（核心已扣）
