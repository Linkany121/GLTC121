    ...
    Bukkit.getPluginManager().registerEvent(ClickEvent, listener, EventPriority.NORMAL, (l, e) => {
        try {
            ...
            if (slot === PREV_SLOT && it.getItemMeta().getDisplayName() === '§e← 上一页') {
                ...
                if (cur > 0) { PAGE_MAP.put(p, cur - 1); const inv = buildSundryMenu(cur - 1); if (inv) openMenu(p, inv); }
                return;
            }
            if (slot === NEXT_SLOT && it.getItemMeta().getDisplayName() === '§e下一页 →') {
                const cur = PAGE_MAP.get(p) || 0;
                const totalPages = Math.ceil(ITEMS['杂物'].length / PAGE_SIZE);
                if (cur < totalPages - 1) { PAGE_MAP.put(p, cur + 1); const inv = buildSundryMenu(cur + 1); if (inv) openMenu(p, inv); }
                return;
            }
...
function openMain(p) { PAGE_MAP.put(p, 0); const inv = buildSundryMenu(0); if (inv) openMenu(p, inv); }
