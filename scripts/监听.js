// ============================================
// GLTC 联合协议 - 低开销版监听
// 优化：早期 return / 先查 buff / 减少 getNearbyEntities / 不用 setHealth
// ============================================
...
var _sweepMethodCache = (function() {
    try {
        return Java.type("org.bukkit.event.entity.EntityDamageByEntityEvent").getMethod("isSweepAttack");
    } catch (e) {
        return null;
    }
})();
...
function hasActiveBuff(player, key) {
    return player.hasMetadata(key) && getEndTime(player, key) > 0;
}
...
function onEntityDamageByEntity(event) {
    ...
    // 走正常伤害管线，避免直接 setHealth 与 CA/EliteMobs 冲突
