    ...
    Bukkit.getPluginManager().registerEvent(ClickEvent, listener, EventPriority.NORMAL, (l, e) => {
        try {
            ...
            if (slot === PREV_SLOT && it.getItemMeta().getDisplayName() === '§e← 上一页') {
                ...
                if (cur > 0) { PAGE_MAP.put(p, cur - 1); const inv = buildDropMenu(cur - 1); if (inv) openMenu(p, inv); else p.sendMessage('§c无法创建兑换菜单，请联系管理员。'); }
                return;
            }
            if (slot === NEXT_SLOT && it.getItemMeta().getDisplayName() === '§e下一页 →') {
                const cur = PAGE_MAP.get(p) || 0;
                const totalPages = Math.ceil(ITEMS['掉落物'].length / PAGE_SIZE);
                if (cur < totalPages - 1) { PAGE_MAP.put(p, cur + 1); const inv = buildDropMenu(cur + 1); if (inv) openMenu(p, inv); else p.sendMessage('§c无法创建兑换菜单，请联系管理员。'); }
                return;
            }
            ...
            // 点击：购买1次；Shift：购买 ceil(64/giveAmount) 次，信用点乘以 BATCH_MULTIPLIER
...
function openMain(p) {
    ...
    PAGE_MAP.put(p, 0); const inv = buildDropMenu(0); if (inv) openMenu(p, inv); else p.sendMessage('§c无法创建兑换菜单，请联系管理员。'); }
